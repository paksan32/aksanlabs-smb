...
## QA — Playwright E2E
Set up 2026-07-15 per the global Playwright standard. `playwright.config.ts` (baseURL = production, `https://smb.aksanlabs.com` — this is a public marketing site with no auth/test accounts, so testing directly against prod is safe). Run via `npm run test:e2e`.
- `e2e/marketing.spec.ts` — homepage sections, client logos, industry pill internal links, FAQ content, sitemap/robots, zero console errors
- `e2e/niche-pages.spec.ts` — all 4 niche pages: unique H1/title, case study content, JSON-LD parses with no `null` entries (automated regression guard for the schema gotcha above)
- `e2e/contact-form.spec.ts` — field presence, `?service=` preselect, native validation. **Intentionally does not submit a real lead** — no dedicated test account exists to isolate test data from production leads, unlike other Aksanlabs projects with authenticated test users. If that changes (e.g. a way to tag/filter test leads in admin.aksanlabs.com), extend this spec to cover a full submit.
